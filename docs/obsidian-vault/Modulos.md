...
- bloques `image` (tipo en `blocks_metadata`: `{type:'image', url, alt, caption}`): en una sección con mapa la tarjeta muestra un thumbnail-trigger y, mientras es el stop activo, el panel sticky muestra la imagen sobre el mapa; en una sección sin mapa se presenta como imagen editorial a ancho completo. En classic se hornea como `<figure class="story-image">`. Las fuentes legacy agrupan sus steps; Organize slides permite intercalar imágenes y texto usando copias guardadas. El editor tiene botón "Image" con upload (mismo pipeline comprimir+`/pages_upload` que las imágenes inline de Quill)
...
## Data Stories: secuencia editorial (2026-09-11)
