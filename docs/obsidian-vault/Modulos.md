...
- shares con stories multi-escena se expanden en "steps": cada escena de la story de Terria se renderiza como un paso dentro de la tarjeta (título + texto escritos en Terria) y el mapa avanza por ellas al hacer scroll (requiere build de Terria con ack `sceneApplied`; si no, el capítulo muestra su escena base por URL — puede aparecer el panel nativo de story de Terria — y los cambios de step no tocan el mapa)
