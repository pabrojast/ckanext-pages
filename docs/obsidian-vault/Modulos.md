...
- cada sección puede tener varios tabs Terria (`#share` o `#start`), mostrados como fuentes de mapa dentro del mismo capítulo; elegir un tab fija esa fuente hasta salir de la sección. El primer tab válido es la fuente inicial y sus shares se resuelven en servidor para precargar los pasos; el resto se resuelve bajo demanda en el navegador
...
- los enlaces `#start` se decodifican y normalizan a `startData` en servidor. Tanto esos datos como los shares pasan por el único mensaje postMessage `applyScene` (replaceStratum, reemplazo limpio de capas), solo mientras el build lo confirme con el ack `sceneApplied`; si no llega ack, los shares degradan a fragment navigation (`#clean&share=...`) y la vía bridge de datos queda desactivada (latch) para el resto de la página. No se reintroduce el mensaje genérico `updateFromStartData`, que no renderiza capas COG en builds estándar de Terria (TypeError `'buffer'` en los workers de geotiff)
...
- una sección sin ningún tab Terria válido se renderiza automáticamente a ancho completo y oculta el panel sticky; el siguiente capítulo con mapa lo vuelve a mostrar y aplica su propia escena, sin heredar visualmente el mapa anterior
