...
- shares con stories multi-escena se expanden en "steps": cada escena de la story de Terria se renderiza como un paso dentro de la tarjeta (título + texto escritos en Terria); al activarse la tarjeta el mapa salta de inmediato a la primera escena del capítulo y luego avanza por los steps al hacer scroll (requiere build de Terria con ack `sceneApplied`; si no, el capítulo muestra su escena base por URL y los cambios de step no tocan el mapa)
- la única vía postMessage de datos es el mensaje `applyScene` (replaceStratum, reemplazo limpio de capas), y solo mientras el build lo confirme con el ack `sceneApplied`; si no llega ack, se degrada a fragment navigation (`#clean&share=...`) y la vía bridge de datos queda desactivada (latch) para el resto de la página. El start-data genérico (`updateFromStartData`) se eliminó: no renderiza capas COG en builds estándar de Terria (TypeError `'buffer'` en los workers de geotiff)
...
- bloques `image` (tipo nuevo en `blocks_metadata`: `{type:'image', url, alt, caption}`): en storymap la tarjeta muestra un thumbnail-trigger y, mientras es el stop activo del scroll, el panel sticky muestra la imagen a pantalla completa sobre el mapa (overlay `.storymap-media-image`, estilo media-slide de ArcGIS); en classic se hornea como `<figure class="story-image">`. Limitación: no se puede intercalar una imagen entre steps de un mismo share (los steps vienen de Terria). El editor tiene botón "Image" con upload (mismo pipeline comprimir+`/pages_upload` que las imágenes inline de Quill)
...
- los scripts `data-stories-storymap.js` y `data-stories-edit.js` se incluyen con `?v=` (cache-bust manual — subirlo al cambiarlos; el editor con JS viejo descartaría silenciosamente tipos de bloque desconocidos al guardar)
