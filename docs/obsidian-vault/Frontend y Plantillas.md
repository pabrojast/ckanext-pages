...
- El embed inline (PDF.js o `<img>`) en `water-publications.html` solo se monta cuando `h.is_ckan_download_url(doc_url)` es `True`, es decir cuando el recurso vive en este CKAN bajo `/dataset/<x>/resource/<y>/download/<file>`. Para links externos (UNESDOC `ark:/…` y similares) se renderiza un panel "Preview not available" y se delega a los botones Read Online / Download, porque PDF.js cross-origin termina recibiendo HTML wrapper y rompe el visor. La helper `is_ckan_download_url` está expuesta desde `plugin.get_helpers` y es un wrapper público sobre `utils._is_ckan_download_url`.
