...
- `data_stories/edit.html` consulta datasets mediante `POST /api/3/action/package_show`, acepta URL, slug o UUID y conserva en `datasets_data` la metadata canónica devuelta por CKAN; el feedback de carga, duplicado o error vive en un contenedor `aria-live` del formulario.
- `data_stories/show_storymap.html` recibe desde `helpers/storymap.py` un layout por sección (`split` o `full`) y una lista de fuentes Terria; `data-stories-storymap.js` reutiliza un solo iframe, permite cambiar de fuente mediante tabs y aplica tanto shares como `startData` normalizado a través del bridge `applyScene`.
